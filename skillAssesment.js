    ...
    return (
      <div style={pageStyle}>
        ...
        <div style={{ maxWidth: 600, margin: '40px auto', textAlign: 'center', padding: '0 20px' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '28px', fontWeight: 700, marginBottom: '12px', background: 'linear-gradient(135deg,#818cf8,#a78bfa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Assessment Complete!</h2>
          <p style={{ color: '#94a3b8', marginBottom: '32px' }}>Your skill profile has been saved. Explore your career recommendations and identify gaps.</p>
    ...
    <div style={pageStyle}>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto 32px', padding: '0 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
          <span style={{ fontSize: '13px', color: '#94a3b8' }}>Step {step + 1} of {totalSteps}</span>
          <span style={{ fontSize: '13px', color: '#818cf8', fontWeight: 600 }}>{progress}% Complete</span>
        </div>
...
function SkillCategoryStep({ category, scores, onScore }) {
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '20px', padding: '28px' }}>
      ...
      <p style={{ color: '#64748b', fontSize: '13px', marginBottom: '24px' }}>Rate your proficiency level for each skill.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {category.skills.map(skill => (
          <div key={skill}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ fontSize: '14px', fontWeight: 500, color: '#e2e8f0' }}>{skill}</span>
              <span style={{ fontSize: '13px', color: '#818cf8', fontWeight: 600 }}>{['None','Beginner','Intermediate','Advanced','Expert'][scores[skill] || 0]}</span>
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              {[0, 1, 2, 3, 4].map(val => (
                <button key={val} onClick={() => onScore(skill, val)} style={{
                  ...
                  borderBottom: (scores[skill] || 0) === val ? '2px solid #818cf8' : '2px solid transparent',
                  transition: 'all 0.2s',
                  fontSize: '11px', color: (scores[skill] || 0) >= val ? '#818cf8' : '#475569',
...
function AcademicStep({ gpa, setGpa, quals, setQuals }) {
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '20px', padding: '28px' }}>
      ...
      <p style={{ color: '#64748b', fontSize: '13px', marginBottom: '24px' }}>Help us generate a more accurate CV and career profile.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div>
...
function NavBtn({ children, onClick, primary }) {
  return (
    <button onClick={onClick} style={{
      ...
      color: primary ? '#fff' : '#818cf8', cursor: 'pointer',
...
const inp = { width: '100%', padding: '12px 16px', background: 'rgba(15,15,42,0.8)', border: '1px solid rgba(79,70,229,0.2)', borderRadius: '10px', color: '#f1f5f9', fontFamily: 'var(--font-body)', fontSize: '14px', outline: 'none' };

function PageHeader({ title, subtitle, navigate, back }) {
  return (
    <div style={{ maxWidth: 700, margin: '0 auto 32px', padding: '0 20px' }}>
      <button onClick={() => navigate(back)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '13px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '6px', padding: 0 }}>
        ← Back to Dashboard
      </button>
