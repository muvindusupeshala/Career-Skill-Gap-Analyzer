...
export default function ProgressTracking({ navigate, user, assessmentData }) {
  ...
  const metrics = [
    ...
    { key: 'programming', label: 'Programming', color: '#4f46e5' },
    { key: 'data', label: 'Data & Analytics', color: '#7c3aed' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-dark)', padding: '40px 20px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 20px' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '32px' }}>Track your skill development over time</p>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '20px', padding: '28px', marginBottom: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px', flexWrap: 'wrap', gap: '12px' }}>
            <div>
              ...
              <p style={{ color: '#64748b', fontSize: '13px', marginTop: '4px' }}>6-month trend</p>
            </div>
            ...
            {history.map((h, i) => {
              ...
              return (
                <div key={h.month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                  ...
                  <div style={{ fontSize: '11px', color: isLast ? '#818cf8' : '#475569', fontWeight: isLast ? 700 : 400 }}>{h.month}</div>
                </div>
              );
            })}
          </div>
              ...
              {Object.entries(assessmentData.skills || {}).map(([skill, score]) => (
                <div key={skill}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '13px' }}>
                    ...
                    <span style={{ color: '#818cf8', fontWeight: 600 }}>{['None','Beginner','Intermediate','Advanced','Expert'][score]}</span>
                  </div>
                  <div style={{ height: 5, background: 'rgba(79,70,229,0.1)', borderRadius: 3 }}>
                    <div style={{ height: '100%', width: `${(score / 4) * 100}%`, background: score >= 3 ? '#10b981' : score >= 2 ? '#818cf8' : '#f59e0b', borderRadius: 3, transition: 'width 0.5s ease' }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px' }}>
            {[
              ...
              <div key={a.title} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px', background: a.earned ? 'rgba(79,70,229,0.12)' : 'rgba(79,70,229,0.04)', border: `1px solid ${a.earned ? 'rgba(129,140,248,0.3)' : 'rgba(79,70,229,0.1)'}`, borderRadius: '12px', opacity: a.earned ? 1 : 0.4 }}>
                ...
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: a.earned ? '#f1f5f9' : '#64748b' }}>{a.title}</div>
